...
export function TopStatusBar() {
    ...
    // Phase to Timeline Logic - Updated with AI-Native terminology
    const timeline = [
    ...
    return (
        <header className={cn(
            "border-b px-6 flex flex-col shadow-sm z-50 relative transition-colors duration-500",
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className={cn("text-sm font-bold uppercase tracking-wider", isEmergency ? "text-red-700" : "text-slate-700")}>
                            {isEmergency ? "EMERGENCY OVERRIDE" : (activeIndex >= 0 ? "AI ADAPTING..." : "SYSTEM STABLE")}
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono">
                            {isEmergency ? "Human-governed emergency response" : "Cognitive broadcast control active"}
                        </div>
                    </div>
                </div>
